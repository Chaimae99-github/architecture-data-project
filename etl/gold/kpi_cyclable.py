...
def compute_kpi_cyclable():
    ...
    if "longueur" in df.columns:
        ...
        len_col="distance"
    elif "longueur_m" in df.columns:
        len_col="longueur_m"
    else:
        raise Exception(
           "colonne longueur absente"
        )
    ...
    if "bidirectionnel" in df.columns:
        agg["bidirectionnel"]="sum"

    if "temporaire" in df.columns:
        agg["temporaire"]="sum"
    ...
    if "bidirectionnel" in df_grouped:
        df_grouped[
           "score_cyclable"
        ]+=(
           0.2*
    ...
    if (
      df_grouped["score"].max()
      !=
      df_grouped["score"].min()
    ):
