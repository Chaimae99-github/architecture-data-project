...
def load_raw_connectivity():
    ...
    mobile_path = RAW_DIR / "antennes-relais.csv"
    wifi_path = RAW_DIR / "sites-disposant-du-service-paris-wi-fi.csv"

    print("🔵 Lecture des fichiers bruts...")

    df_mobile = pd.read_csv(
        mobile_path,
        sep=";"
    )
    ...
    print(
        f"✅ Antennes: {df_mobile.shape}"
    )

    print(
        f"✅ Wifi: {df_wifi.shape}"
    )
...
def clean_mobile(df_mobile):
    ...
    lat_col = next(
      (
       c for c in df_mobile.columns
       if c in possible_lat
      ),
      None
    )
    ...
    if lat_col is None or lon_col is None:
        ...
        if "geo_point_2d" in df_mobile.columns:
            ...
            raise ValueError(
               "Colonnes coordonnées introuvables"
            )
    ...
    gdf_mobile = gdf_mobile.to_crs(
       2154
    )

    print(
      f"✅ {len(gdf_mobile)} antennes nettoyées"
    )
...
def clean_wifi(df_wifi):
    ...
    gdf_wifi=gdf_wifi.to_crs(
       2154
    )

    print(
      f"✅ {len(gdf_wifi)} hotspots wifi"
    )
...
# =====================================================
# AGGREGATION KPI PAR RUE
# =====================================================

def aggregate_by_street(
   gdf_mobile,
   gdf_wifi
):
    ...
    print(
      f"✅ {merged.shape[0]} rues agrégées"
    )
...
def main():
    ...
    df_mobile,df_wifi=(
      load_raw_connectivity()
    )

    gdf_mobile=clean_mobile(
      df_mobile
    )
    ...
    print(
      "🔵 Jointure mobile -> rues"
    )

    gdf_mobile=(
      spatial_join_with_streets(
         gdf_mobile,
         streets
      )
    )
