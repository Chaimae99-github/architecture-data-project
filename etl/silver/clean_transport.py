...
Entrées :
- data/bronze/voies_paris_raw.json
...
print("=== CLEAN TRANSPORT VERSION MODIFIEE ===")
...
def point_linestring_distance_m(lon, lat, coords):
    if len(coords) < 2:
        return float("inf")
...
def point_in_expanded_bbox(lon, lat, bbox, expand_m):
...
def clean_voies(raw_path):
    logger.info("Nettoyage voies...")
 
    with open(raw_path, encoding="utf-8") as f:
        wrapper = json.load(f)
 
    features = wrapper["data"]["features"]
    cleaned = []
    ...
    for feat in features:
        ...
        if len(cleaned) == 0:
            print("Colonnes disponibles dans props :", list(props.keys()))
        ...
        nom = (
            safe_str(props.get("l_voie"))
            or safe_str(props.get("l_longmin"))
            or safe_str(props.get("l_courtmin"))
            or safe_str(props.get("libelle"))
    ...
    logger.info(f"✓ {len(cleaned)} voies")
    return cleaned
...
def parse_modes(props):
    ...
    for col, mode in bool_map.items():
        val = str(props.get(col, "")).lower().strip()
 
        if val in ("1", "true", "oui", "yes"):
            modes.append(mode)
 
    if not modes:
        raw_mode = safe_str(props.get("mode")).upper()
 
        for m in ["RER", "TRAIN", "METRO", "TRAM", "VAL"]:
            if m in raw_mode:
                modes.append(m)
 
    return list(dict.fromkeys(modes)) or ["UNKNOWN"]
 
 
def clean_gares(raw_path):
    logger.info("Nettoyage gares...")
    ...
    logger.info(f"✓ {len(cleaned)} gares")
    return cleaned
...
def spatial_join(voies, gares, radius_m=RADIUS_M):
    joined = []
    total = len(voies)
 
    for i, voie in enumerate(voies, 1):
        if i % 1000 == 0:
            logger.info(f"{i}/{total} voies")
 
        vp = voie["properties"]
        coords = get_all_coords(voie["geometry"])
...
def save_geojson(features, path, label):
    geojson = {
...
def run():
    ...
    # --------------------------------
    # Construire dataframe KPI transport
    # --------------------------------
    ...
    print(
        "Transport avant mapping:",
        df.shape
    )

    # --------------------------------
    # Mapping spatial vers streets_base
    # --------------------------------
    ...
    streets = gpd.read_parquet(
        os.path.join(
            BRONZE_DIR,
            "streets_base.parquet"
        )
    )

    # voies transport -> geodataframe
    voies_gdf = gpd.GeoDataFrame.from_features(
        joined
    )

    # définir CRS source
    voies_gdf = voies_gdf.set_crs(
        "EPSG:4326"
    )

    # reprojection comme streets
    voies_gdf = voies_gdf.to_crs(
        streets.crs
    )

    print(
        "Jointure spatiale vers référentiel rues..."
    )
    ...
    matched = mapped["street_id_right"].notna().sum()
    total = len(df)

    rate = matched / total * 100

    print(
    f"Matching: {matched}/{total} ({rate:.2f}%)"
    )

    # IMPORTANT
    df["street_id"] = (
        mapped["street_id_right"]
        .astype(int)
        .values
    )
